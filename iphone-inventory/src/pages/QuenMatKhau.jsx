...
export default function QuenMatKhau() {
  ...
  const handleSendOTP = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${process.env.VITE_API_URL || 'http://localhost:4000'}/api/auth/send-reset-otp`, {
        method: "POST",
  ...
  const handleVerifyOTP = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${process.env.VITE_API_URL || 'http://localhost:4000'}/api/auth/verify-reset-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
