...
function BaoCao() {
  ...
  const loadBranches = async () => {
    try {
      ...
      if (response.ok && data.length > 0) {
        ...
        const branchNames = data.map(b => typeof b === 'string' ? b : (b.name || b));
  ...
  const fetchData = async (fromDate, toDate, branchParam, categoryParam) => {
    ...
    try {
      ...
      let api = getApiUrl('/api/report/bao-cao-loi-nhuan');
      if (fromDate && toDate) {
      ...
      if (fromDate && toDate) {
        ...
        const urlFinancial = `${getApiUrl('/api/report/financial-report/summary')}?from=${fromDate}&to=${toDate}&branch=${branchParam}${categoryQueryFinancial}`;
        const resFinancial = await fetch(urlFinancial, { 
          headers: { 
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          } 
